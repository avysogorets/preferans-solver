...
def solver(hands,turn):
    ...
    for card in hands[turn].options(hands[3].cards[0]):
        ...
        if game_params['game_type']=='P':
            if game_params['player']==turn and ((hands[3].cards[-1].suit is not None and current_objective<new_tricks[game_params['player']]) or (hands[3].cards[-1].suit is None and current_objective<=new_tricks[game_params['player']])):
                current_objective=new_tricks[game_params['player']]
                dp[key_string]={'card':card.to_string,'turn':turn,'num_tricks':new_tricks}
            if game_params['player']!=turn and ((hands[3].cards[-1].suit is not None and current_objective<100-new_tricks[game_params['player']]) or (hands[3].cards[-1].suit is None and current_objective<=100-new_tricks[game_params['player']])):
